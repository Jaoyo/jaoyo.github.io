...
2. 
    ...
    这些指令告诉 `CPP` 从系统库中获取 s`tdio.h`，并添加文本到当前的源文件中。下一行告诉 `CPP` 从本地目录中获取 `myheader.h`，并添加内容到当前的源文件中。
...
此外，可以使用`gcc -E`去查看预处理之后的结果，此例展开后的代码如下：
...
如果在`#if !defined (MESSAGE)`定义了`MESSAGE`，则不会再定义`MESSAGE`
...
在使用带有参数的宏之前，必须使用 `#define` 指令定义。参数列表是括在圆括号内，且必须紧跟在宏名称的后边。宏名称和左圆括号之间**不允许有空格**。例如：
